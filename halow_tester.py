...
def _get_peer_stats_raw(session_token: str, id_counter: int) -> dict:
    peer_status_request = {
        'jsonrpc': '2.0',
        'id': id_counter,
        'method': 'call',
    ...
    if peer_status_response['result'][0] != 0:
        raise Exception('Invalid response from OpenWRT UBUS')

    return peer_status_response['result'][1]['results'][0]

def get_bandwidth(session_token: str, id_counter: int) -> int:
    peer_stats = _get_peer_stats_raw(session_token, id_counter)
...
def get_peer_stats(session_token: str, id_counter: int) -> tuple:
    peer_stats_raw = _get_peer_stats_raw(session_token, id_counter)
...
def get_iperf3_windows(bandwidth: int) -> str:
    return str(IPERF3_TCP_TEST_WINDOWS[int(math.log(bandwidth, 2))])
...
def main() -> None:
    ...
    id_counter = 0
    bandwidth = get_bandwidth(session_token, id_counter := id_counter + 1)
    ...
    #         stat_log.append(get_peer_stats(session_token, id_counter := id_counter + 1))
    #         rssi = stat_log[-1][1]
    #         noise = stat_log[-1][3]
    #         snr = rssi - noise
    ...
    #         print(f'\033[2K\033[?25l{PROGRESS_SPIN[spinner_index]} Performing TCP test [{i + 1}/{IPERF3_TCP_TEST_COUNT}] (RSSI: {rssi}dBm, Noise Floor: {noise}dBm, SNR: {snr}dB, Previous Bitrate: {previous_tcp_bitrate})', end='\r')
    #         spinner_index = (spinner_index + 1) % len(PROGRESS_SPIN)
    ...
    # print(f'\033[2K✓ TCP Testing Complete (Average Bitrate: {mean(previous_tcp_bitrates):.2f} Kbit/s)')
    ...
    if ping_process.stdout is None:
        print('ERROR: Failed to construct stdout pipe for ICMP test. Terminating.')
        sys.exit(-1)
    ...
    while ping_process.poll() == None:
        ...
        print(f'\033[2K\033[?25l{PROGRESS_SPIN[spinner_index]} Performing ICMP test (RSSI: {rssi}dBm, Noise Floor: {noise}dBm, SNR: {snr}dB)', end='\r')
        spinner_index = (spinner_index + 1) % len(PROGRESS_SPIN)
